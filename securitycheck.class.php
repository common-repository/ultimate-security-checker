...
class SecurityCheck {
    ...
    private function set_vars(){
        ...
        $this->all_issues = array(
            ...
            array(
                'id' => 27,
                'title' => __('Default wp login url is not safe.'),
                'points' => 5,
                'category' => 'login-page',
            ...
            array(
                'id' => 28,
                'title' => __('Your Database connetion password is not safe.'),
                'points' => 5,
                'category' => 'db',
                'callback' => 'run_test_28'
            ),
    ...
    public function run_test_3(){
        if( version_compare( $this->_wp_version, '2.92', '>' ) ){
    ...
    public function run_test_5(){
        ...
        if( $this->_wp_version > 2.6 ){
          //if version > 2.6
           $consts = array( 'AUTH_KEY', 'SECURE_AUTH_KEY', 'LOGGED_IN_KEY', 'AUTH_SALT', 'SECURE_AUTH_SALT', 'LOGGED_IN_SALT' );
          foreach ( $consts as $const ) {
            if( defined( $const ) && in_array( trim( constant( $const ) ), array('put your unique phrase here', '') ) ) {
              $keys_absent[] = $const;
            }
          }
        }
        if( $this->_wp_version > 2.7 ){
          //if version > 2.7
          $consts = array( 'NONCE_KEY', 'NONCE_SALT' );
            foreach ( $consts as $const ) {
              if( defined( $const ) && in_array( trim( constant( $const ) ), array('put your unique phrase here', '') ) ) {
                $keys_absent[] = $const;
            }
    ...
    public function run_test_9(){
        if( version_compare( $this->_wp_version, '4.8.1', '<' ) ){
    ...
    public function run_heuristic_check() {
		...
		'/\$_COOKIE\[\'yahg\'\]/i' => __('YAHG Googlerank.info exploit code at creativebriefing.com'),
    ...
    public function run_test_27() {
      $wp_login_url = wp_login_url();
      $url_parts = explode( '/', $wp_login_url );
      $slug = end( $url_parts );
    ...
    public function check_password_strength( $pwd ) {
    ...
    public function run_test_28() {
      $db_pwd = DB_PASSWORD;
      if( ! $this->check_password_strength( $db_pwd ) ) {
          return False;
      }
      return True;
    }
